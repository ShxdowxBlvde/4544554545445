...
def test_filter_by_currency_success(sample_transactions: List[Dict[str, Any]]) -> None:
    """Проверка корректности фильтрации транзакций по заданной валюте"""
    usd_transactions: List[Dict[str, Any]] = list(
...
def test_filter_by_currency_rub(sample_transactions: List[Dict[str, Any]]) -> None:
    ...
    rub_transactions: List[Dict[str, Any]] = list(
        filter_by_currency(sample_transactions, "RUB")
    )
...
def test_filter_by_currency_no_match(sample_transactions: List[Dict[str, Any]]) -> None:
    """Проверка случая, если транзакции в заданной валюте отсутствуют (например, GBP)"""
...
def test_transaction_descriptions_success(
    ...
    expected: List[str] = [
        ...
        "Покупка в магазине",
        "",  # Для транзакции 999999999 (отсутствует описание)
        "",  # Для транзакции 111111111 (отсутствует описание)
    ]
...
def test_card_number_generator_formatting() -> None:
    ...
    generator: Generator[str, None, None] = card_number_generator(123456, 123456)
    card_str: str = next(generator)
